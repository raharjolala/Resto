    ...
    <div class="content-card">
        <div class="card-header">
            ...
            <p>
                ...
                Admin / <span style="color: var(--red-500); font-weight: 600;">Edit Menu</span>
            </p>
        </div>
        ...
        <form action="{{ route('admin.menu.update', $menuItem->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          ...
                          name="description" 
                          rows="3" 
                          placeholder="Jelaskan detail menu...">{{ old('description', $menuItem->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        inputs.forEach(input => {
            input.addEventListener('focus', function() {
                this.closest('.form-group')?.querySelector('label')?.classList.add('text-danger');
            });
            input.addEventListener('blur', function() {
                this.closest('.form-group')?.querySelector('label')?.classList.remove('text-danger');
            });
        });
    });
    ...
    document.querySelector('button[type="reset"]').addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Reset image preview
                const imageInput = document.getElementById('image');
                imageInput.value = '{{ $menuItem->image }}';
                updatePreview(imageInput.value);
                
                Swal.fire({
                    icon: 'success',
                    title: 'Form di-reset!',
                    text: 'Form kembali ke data awal',
                    timer: 1500,
                    showConfirmButton: false
                });
            }
